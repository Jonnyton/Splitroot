...
$harnessRoot = Join-Path $proofDir 'HostSupervisorExitRefreshProof'
$jsonPath = Join-Path $proofDir 'last-host-supervisor-exit-refresh-proof.json'
$stdoutPath = Join-Path $proofDir 'host-supervisor-exit-refresh-proof.stdout.log'
$stderrPath = Join-Path $proofDir 'host-supervisor-exit-refresh-proof.stderr.log'
...
if (Test-Path -LiteralPath $harnessRoot) {
    Stop-HarnessProcessFromState (Join-Path $harnessRoot 'Saved\Proof\playtest-drive-session.json')
...
$fakeSourcePath = Join-Path $fakeSourceDir 'ArchonExitRefreshProof.cpp'
Set-Content -LiteralPath $fakeDllPath -Value 'fake old module dll for host-exit refresh proof' -Encoding UTF8
...
$fakeBuildPath = Join-Path $fakeDriveDir 'fake-build-at-host-exit.ps1'
@'
param(
    [string]$ProjectRoot = ''
)
...
} | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $proofDir 'fake-host-exit-build.json') -Encoding UTF8
exit 0
...
$deadHost = Start-Process -FilePath 'powershell' -ArgumentList @('-NoProfile','-Command','Start-Sleep -Seconds 600') -WindowStyle Hidden -PassThru
$deadPid = [int]$deadHost.Id
...
$oldFingerprint = "ArchonFactoryCanary: BuildFingerprint moduleDllUtc=$oldDllText processStartUtc=$oldProcessStart modulePath=$fakeDllPath version=vproof-old effectiveUtc=$oldProcessStart"
$fakeLogPath = Join-Path $fakeProofDir 'playtest-drive.log'
@(
    $oldFingerprint,
    'ArchonFactoryCanary: MatchLoopActive sites=3 proof=true'
) | Set-Content -LiteralPath $fakeLogPath -Encoding UTF8
...
[pscustomobject]@{
    ...
    PlaytestStatePath = Join-Path $fakeProofDir 'playtest-drive-session.json'
    PlaytestLogPath = $fakeLogPath
    AllowBoundaryRefresh = $true
    BuildAtBoundary = $true
    BuildScriptPath = $fakeBuildPath
} | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $fakeProofDir 'host-supervisor-session.json') -Encoding UTF8

powershell -ExecutionPolicy Bypass -File (Join-Path $fakeDriveDir 'host-refresh-pending-state.ps1') `
    -Action write `
    -ProjectRoot $harnessRoot `
    -Reason 'source_newer_than_module_boundary_build_pending' `
    -CurrentProcessId $deadPid `
    -CurrentBuildFingerprint $oldFingerprint `
    -CurrentModuleDllUtc $oldDllText `
    -PendingModuleDllUtc $oldDllText `
    -NextMap 'splitroot_valley' `
    -SupervisorOwnsHost $true `
    -SupervisorMode 'proof_seed_supervisor_owned_host_exited' `
    -ObservedLogPath $fakeLogPath | Out-Null
...
if (-not $supervisorProc.WaitForExit(30000)) {
    ...
    throw 'Host supervisor exit-refresh proof timed out.'
}
...
$historySawExitRefresh = $pendingHistoryText -match 'fresh_build_host_exit_refresh' -and $pendingHistoryText -match 'host_refreshing_build_reconnecting'
...
$result = [pscustomobject]@{
    ...
    RelaunchedAfterExitedHost = [bool]($refreshProof -and $refreshProof.BeforePid -eq $deadPid -and $refreshProof.AfterPid -and $refreshProof.AfterPid -ne $deadPid)
    PendingStateEmitted = [bool]$historySawExitRefresh
...
if (
    ...
    $result.ProcessBoundary -ne 'host_exit_fresh_process_relaunch' -or
    $result.RefreshTrigger -ne 'host_exit_pending_refresh' -or
    -not $result.RelaunchedAfterExitedHost -or
